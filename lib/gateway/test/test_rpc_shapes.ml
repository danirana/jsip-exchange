...
let%expect_test "login_rpc shape" =
  let description = Rpc.Rpc.description Rpc_protocol.login_rpc in
...
let%expect_test "session_feed_rpc shape" =
  let description = Rpc.Pipe_rpc.description Rpc_protocol.session_feed_rpc in
  print_s [%sexp (description : Rpc.Description.t)];
  [%expect {|
    ((name session-feed) (version 1))
  |}];
  return ()
;;

let%expect_test "cancel_order_rpc shape" =
  let description = Rpc.Rpc.description Rpc_protocol.cancel_order_rpc in
  print_s [%sexp (description : Rpc.Description.t)];
  [%expect {| 
    ((name cancel-order) (version 1))
  |}];
  return ()
;;
...
let%expect_test "stats RPC" =
  ...
  [%expect
    ...
    (Streaming_rpc (query 86ba5df747eec837f0b391dd49f33f9e)
     (initial_response 86ba5df747eec837f0b391dd49f33f9e)
     (update_response 5bc49a328d02d6badcc1168dead29149)
     (error 52966f4a49a77bfdff668e9cc61511b3))
    |}];
  return ()
;;
...
(* Digest-level shape tests for the session RPCs added in Part 2. These pin
   ...
   reconciliation compares. (The [description]-based blocks above pin only
