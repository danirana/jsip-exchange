      ...
      (match verb with
       ...
       | Cancel ->
         (match rest with
          | [ cl_id_str ] ->
            (try
               Or_error.return (Cancel (Client_order_id.of_string cl_id_str))
             with
             | _ -> Or_error.errorf "invalid client order ID: %s" cl_id_str)
          | _ -> Or_error.error_string "expected: CANCEL <client_order_id>")
       (* if verb is Buy or Sell, it expects client id, symbol, size, price,
          and time in force *)
       | Buy | Sell ->
         (match rest with
          | client_order_id_str
            ...
            let%bind client_order_id =
              try
                Or_error.return
                  (Client_order_id.of_string client_order_id_str)
              with
              | _ ->
                Or_error.errorf
                  "invalid client order ID: %s"
                  client_order_id_str
            in
            let%bind size =
          ...
          | _ ->
            Or_error.errorf
              "expected: %s <client_order_id> <symbol> <size> <price> %s \
               [as <name>]"
              (String.uppercase verb_str)
              Time_in_force.all_str)
       (* if verb is Book or Subscribe, it expects symbol *)
       | Book | Subscribe ->
         (match rest with
          | symbol_str :: rest_of_args ->
            if not (List.is_empty rest_of_args)
            then
              Or_error.errorf
