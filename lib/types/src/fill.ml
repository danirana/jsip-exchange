...
let to_string ({ fill_id ; symbol ; price ; size ; aggressor_order_id ; aggressor_participant ; aggressor_client_order_id ; aggressor_side ; resting_order_id ; resting_participant ; resting_participant_client_order_id } : t) =
  sprintf "fill_id=%d %s %s x%d aggressor=%s(%s, cid=%s) %s resting=%s(%s, cid=%s)" 
    fill_id 
...
let to_participant_view t specific_participant = 
  let participant_side = 
    if Participant.equal t.aggressor_participant specific_participant then
      Some t.aggressor_side
    else if Participant.equal t.resting_participant specific_participant then
      Some (Side.flip t.aggressor_side)
    else
      None
  in
  Option.map participant_side ~f:(fun side ->
    ...
    [%string "You %{action} %{action_size#Int} %{symbol_str} at %{price_str}"]
  )
;;
