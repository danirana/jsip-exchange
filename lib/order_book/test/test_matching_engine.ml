...
let%expect_test "exact cross at same price" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x100 aggressor=2(Alice, cid=1) BUY resting=1(Bob, cid=1)
    |}]
;;
...
let%expect_test "partial fill: buy is larger than resting sell" =
  ...
  [%expect
    ...
    ACCEPTED id=2 AAPL BUY 100@$150.00 DAY
    FILL fill_id=1 AAPL $150.00 x60 aggressor=2(Alice, cid=1) BUY resting=1(Bob, cid=1)
    |}];
...
let%expect_test "aggressor sweeps multiple resting orders" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x50 aggressor=3(Alice, cid=1) BUY resting=1(Bob, cid=1)
    FILL fill_id=2 AAPL $150.00 x50 aggressor=3(Alice, cid=1) BUY resting=2(Charlie, cid=1)
    |}]
;;
...
let%expect_test "IOC: no match means immediate cancel" =
  ...
  [%expect
    ...
    CANCELLED id=1 AAPL remaining=100 reason=IOC_REMAINDER cid=1
    |}]
;;
...
let%expect_test "IOC: partial fill then cancel remainder" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x40 aggressor=2(Alice, cid=1) BUY resting=1(Bob, cid=1)
    CANCELLED id=2 AAPL remaining=60 reason=IOC_REMAINDER cid=1
    |}]
;;
...
let%expect_test "price priority: naive impl matches first-found, not best" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $10.00 x100 aggressor=3(Alice, cid=1) BUY resting=1(Charlie, cid=1)
    |}]
;;
...
let%expect_test "time priority: equal-priced bids fill oldest-first" =
  ...
  [%expect
    ...
    ACCEPTED id=1 AAPL BUY 100@$150.00 DAY
    ACCEPTED id=2 AAPL BUY 100@$150.00 DAY
    ACCEPTED id=3 AAPL SELL 100@$150.00 DAY
    FILL fill_id=1 AAPL $150.00 x100 aggressor=3(Charlie, cid=1) SELL resting=1(Alice, cid=1)
    |}]
;;
...
let%expect_test "scenario: two participants trade, book reflects state" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.10 x50 aggressor=5(Charlie, cid=1) BUY resting=3(Bob, cid=1)
    === AAPL ===
      BIDS:
        $149.90 x100
        $149.80 x200
...
let%expect_test "scenario: aggressive IOC sweeps entire book" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x50 aggressor=4(Alice, cid=1) BUY resting=1(Bob, cid=1)
    FILL fill_id=2 AAPL $150.10 x50 aggressor=4(Alice, cid=1) BUY resting=2(Charlie, cid=1)
    FILL fill_id=3 AAPL $150.20 x50 aggressor=4(Alice, cid=1) BUY resting=3(Bob, cid=2)
    CANCELLED id=4 AAPL remaining=50 reason=IOC_REMAINDER cid=1
    === AAPL ===
      BIDS: (empty)
      ASKS: (empty)
      BBO: - / -
...
let%expect_test "scenario: fill IDs are globally sequential" =
  ...
  [%expect
    ...
    ACCEPTED id=2 TSLA SELL 100@$200.00 DAY
    ACCEPTED id=3 AAPL BUY 100@$150.00 DAY
    FILL fill_id=1 AAPL $150.00 x100 aggressor=3(Alice, cid=1) BUY resting=1(Bob, cid=1)
