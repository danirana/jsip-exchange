...
let mark_id_as_terminal t participant client_order_id = 
  ...
  | Some id_table -> 
    ...
    if Hashtbl.mem id_table raw_cl_id then 
      Hashtbl.set id_table ~key:raw_cl_id ~data:None 
;;

(** Run the matching loop: repeatedly find a compatible resting order and fill against it. 
    Returns the list of Fill and Trade_report events produced, and the next fill_id to use. *)
let rec match_loop ~engine ~book ~order ~fill_id = 
  if Size.( <= ) (Order.remaining_size order) Size.zero then 
    [], fill_id 
  else (
    match Order_book.find_match book order with 
    | None -> [], fill_id 
    | Some resting -> 
      let fill_size = Size.min (Order.remaining_size order) (Order.remaining_size resting) in 
      Order.fill order ~by:fill_size; 
      Order.fill resting ~by:fill_size; 
      
      if Order.is_fully_filled resting then (
        Order_book.remove book (Order.order_id resting);
        mark_id_as_terminal engine (Order.participant resting) (Order.client_order_id resting)
      ); 
      ...
      let fill_event = 
        Exchange_event.Fill { 
          ...
          resting_participant_client_order_id = Order.client_order_id resting 
        } 
      in 
      let trade_event = 
      ...
      in 
      let remaining_events, next_fill_id = match_loop ~engine ~book ~order ~fill_id:(fill_id + 1) in 
...
let submit t (request : Order.Request.t) = 
  let id_table = 
    Hashtbl.find_or_add t.seen_client_order_ids request.participant 
  ...
  if Hashtbl.mem id_table raw_cl_id then 
    [ Exchange_event.Order_reject { request; reason = "duplicate client_order_id" } ] 
  else (
    match Map.find t.books request.symbol with 
    | None -> [ Exchange_event.Order_reject { request; reason = "unknown symbol" } ] 
    | Some book -> 
      ...
      (* Match *) 
      let fill_events, next_fill_id = match_loop ~engine:t ~book ~order ~fill_id:t.next_fill_id in 
      ...
      let bbo_events = 
        if Bbo.equal bbo_before bbo_after then [] 
        else [ Exchange_event.Best_bid_offer_update { symbol = Order.symbol order; bbo = bbo_after } ] 
      in 
...
let cancel t participant client_order_id = 
  ...
  | None -> 
    [ Exchange_event.Cancel_reject { participant; client_order_id; reason = "order not found" } ] 
  | Some order -> 
    let symbol = Order.symbol order in 
    match Map.find t.books symbol with 
    | None -> 
      [ Exchange_event.Cancel_reject { participant; client_order_id; reason = "unknown symbol" } ] 
    | Some book -> 
      let bbo_before = Order_book.best_bid_offer book in 
      ...
      let bbo_events = 
        if Bbo.equal bbo_before bbo_after then [] 
        else [ Exchange_event.Best_bid_offer_update { symbol; bbo = bbo_after } ] 
      in 
