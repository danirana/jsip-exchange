...
module Market_maker_bot : sig
  ...
  val seed_book : Config.t -> Context.t -> fair_value_cents:int -> unit Deferred.t
  val update_inventory : Fill.t -> is_aggressor:bool -> fill_size:int -> int
  val update_active_orders : my_client_order_id:Client_order_id.t -> fill_size:int -> unit
  val cancel_and_re_quote : Config.t -> Context.t -> new_inventory:int -> unit
