...
(* let%expect_test "seed_book: places symmetric bids and asks around fair value"
  ...
  with_server ~symbols:[ Harness.aapl ] (fun ~server:_ ~port ->
    ...
    let%bind () = Market_maker.seed_book default_config (connection mm) ~fair_value_cents:default_config.fair_value_cents in
    [%expect {|
...
(* 
let%expect_test "2a test" = 
  let print_current_state () =
    let inv_val = Option.value (Hashtbl.find inventory Harness.aapl) ~default:0 in
    let live_ids = Hashtbl.keys active_orders 
                   |> List.map ~f:Client_order_id.to_int 
  ...
  print_endline "--- Initial Clean State ---";
  print_current_state ();

  (*an Order_accept Event arriving on the session feed *)
  print_endline "\n--- Event 1: Market Maker Order Accepted ---";
  let my_order_id = Client_order_id.of_int 42 in
  Hashtbl.set active_orders ~key:my_order_id ~data:100;
  print_current_state ();
