...
let%expect_test "seed_book: places symmetric bids and asks around fair value"
  ...
  with_server ~symbols:[ Harness.aapl ] (fun ~server:_ ~port ->
    ...
    [%expect
      ...
      [MarketMaker] REJECTED AAPL SELL 100@$150.10 reason=duplicate client_order_id
      [MarketMaker] REJECTED AAPL BUY 100@$149.89 reason=duplicate client_order_id
      [MarketMaker] REJECTED AAPL SELL 100@$150.11 reason=duplicate client_order_id
      [MarketMaker] REJECTED AAPL BUY 100@$149.88 reason=duplicate client_order_id
      [MarketMaker] REJECTED AAPL SELL 100@$150.12 reason=duplicate client_order_id
      |}];
    return ())
;;
